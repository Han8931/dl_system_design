...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.4]{./images/basic_llm_service.png}
\end{figure}
...
\begin{itemize}
	\item Fixed window: allow a fixed number of requests in a set duration of time.
		\begin{itemize}
			\item Problem: Burst at window boundaries:
				\begin{itemize}
					\item User makes 100 requests at 12:59 PM
					\item User makes 100 requests at 1:00 PM
					\item Result: 200 requests in 1 minute! (violates the spirit of the limit)
				\end{itemize}
		\end{itemize}
	\item Sliding window log: Keep a log of all request timestamps. For each new request, remove timestamps older than the window duration, then check if we're under the limit.
		\begin{itemize}
			\item Example: Allow 100 requests per hour
				\begin{itemize}
					\item Maintain a list of timestamps: [12:05, 12:10, 12:15, ...]
					\item New request at 1:20 PM → Remove all timestamps before 12:20 PM
					\item If remaining count < 100, allow the request
				\end{itemize}
		\end{itemize}
	\item Token bucket: Clients initially have a full bucket of tokens, and with each request, they spend tokens. When the bucket is empty, the requests are blocked. 
	\item Leaky bucket: Requests enter a queue (bucket). Queue is processed at constant rate. If queue is full, new requests are rejected.
\end{itemize}
\end{commentbox}
...
Who is responsible for the feature? What is its definition? Who can access it? Let's take a look at setting one up and querying the data to get a feel for how they work. We'll be using \textsc{Feast}, which is open source and supports a variety of backends. To get started, let us \texttt{pip install feast} and then run the \texttt{init} command in your terminal to set up a project, like so:
...
At its core, k8s works by grouping containers into logical units called \textit{pods}, which are the smallest deployable units in the k8s ecosystem. These \textbf{pods are then scheduled and managed by the k8s control plane} (\ie the brain of Kubernetes), which oversees their deployment, scaling, and updates. This control plane consists of several components that collectively handle the orchestration and management of containers. 

\begin{figure}[h]
	\centering
	\includegraphics[scale=0.4]{./images/simple_k8s.png}
\end{figure}

Control Plane Node vs Worker Node:
\begin{itemize}
	\item Control plane nodes are dedicated to running Kubernetes system components that manage the cluster
		\begin{itemize}
		...
		\end{itemize}
	\item Worker nodes run your application workloads. Each node is a computer (physical or virtual). It runs:
		\begin{itemize}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.4]{./images/basic_hpa.png}
	\caption{Basic autoscaling using the in-built k8s HPA. The HPA watches CPU and memory resources and will tell the deployment service to increase or decrease the number of replicas.}
\end{figure}
...
\begin{itemize}
	\item Prometheus is a time-series database that collects and stores metrics.
	\item Grafana is a dashboarding and visualization tool that queries Prometheus (and other data sources) to create visual representations.
\end{itemize}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.4]{./images/hpa.png}
	\caption{Autoscaling based on a custom metric like GPU utilization. }
	\label{fig:}
\end{figure}
...
\begin{itemize}
	...
	\item Min pod replicas
	\item Max pod replicas
...
Another concern when selecting the metric is its stability. For example, an indi- vidual GPU's utilization tends to be close to either 0\% or 100\%. This can cause problems for the autoscaler, as the metric oscillates between an on and off state, as will its recommendation to add or remove replicas, causing flapping. Generally, flapping is avoided by taking the average utilization across all GPUs running the service. Using the average will stabilize the metric when you have a lot of GPUs, but it could still be a problem when the service has scaled down. If you are still running into problems, you'll want to use an average-over-time aggregation, which will tell you the utilization for each GPU over a time frame—say, the last 5 minutes. For CPU utilization, average-over-time aggregation is built into the Kubernetes HPA and can be set with the horizontal-pod-autoscaler-cpu-initialization-period flag. For custom metrics, you'll need to set it in your metric query (for Prometheus, it would be the \texttt{avg\_over\_time} aggregation function).


\subsection{Rolling Updates}

\textbf{Rolling updates or rolling upgrades is a strategy that gradually implements the new version of an application to reduce downtime and maximize agility}. It works by \textbf{gradually creating new instances and turning off the old ones}, replacing them in a methodical manner. This update approach allows the system to remain functional and accessible to users even during the update process, otherwise known as zero downtime. Rolling updates also make it easier to catch bugs before they have too much effect and rollback faulty deployments. 
